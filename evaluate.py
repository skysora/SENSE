...
def calculate_warning_times(config, model_list, data, event_metadata, batch_size, sampling_rate=100,
    ...
    for i, _ in tqdm(enumerate(event_metadata.iterrows()), total=len(event_metadata)): #分別進入每個事件
        ...
        predictions = model_list.predict_generator(cutout_generator, workers=workers, use_multiprocessing=use_multiprocessing)  # (時間, (trace(超過20測站的trace會有>1個trace), 20, 50, 3))
        ...
        # print('predictions',predictions.shape)
        pga_pred = []
        for station in generator.metadata[0]:
            station_key = f"{station[0]},{station[1]},{station[2]}"
            postion = stations_table[station_key]
            pga_pred.append(predictions[:,postion,:,:])
            
        pga_pred = torch.Tensor(pga_pred).permute((1, 0, 2,3))
        
        # print(f"pga_pred:{pga_pred.shape}")
        ...
        #print('pga_times_true_pre',pga_times_true_pre)
...
if __name__ == '__main__':
    ...
    if args.val:
        ...
        training_params['data_path'] = "/home/sora/M11015203/docker-home/earthquake/data/taiwan_val_v3.hdf5"
        test_set = False
    else:
        output_dir = os.path.join(args.experiment_path, f'evaluation{suffix}', 'test_6')
        training_params['data_path'] = "/home/sora/M11015203/docker-home/earthquake/data/taiwan_test_v3.hdf5"
        test_set = True

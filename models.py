...
class PositionEmbedding(nn.Module): #emb_dim=500
    ...
    def forward(self, x):
        x = x.to(self.rotation_matrix.device)
        fake_borehole = False
...
class WaveformsEmbedding(nn.Module):  
    ...
    def forward(self, x, waveforms_mask):
        ...
        x = x * waveforms_mask.to(x.device)
...
class TotalEmbedding(nn.Module):
    def __init__(self, input_shape, downsample, mlp_dims=(500,500,500), wavelength=[[0.1,25],[0.1,25],[0.01,10]], device='cpu',
        ...
        nn.init.zeros_(self.SegmentEmbedding.weight.data)
        self.w = torch.nn.parameter.Parameter(data=torch.ones(250, 2), requires_grad=True)
        
    def forward(self, waveforms, inputs,embedding_input): #(64, 25, 3000, 6)
        ...
        segmentEmbedding = self.SegmentEmbedding(embedding_input)
        w = self.w
        if not self.alternative_coords_embedding:
            inputs = self.Input_Station_emb(inputs).to(waveforms.device)
            inputs = inputs * inputs_mask.to(waveforms.device)
            outputs = torch.mul(w[:,0].unsqueeze(-1),waveforms) + torch.mul(w[:,1].unsqueeze(-1),inputs)
...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, x, att_mask, station_mask):
        ...
        inv_mask = torch.unsqueeze(torch.logical_not(station_mask),-1)  
        mask_B = inv_mask.permute((0, 2, 3, 1))  
        score = score.to(x.device) - mask_B.to(x.device) * self.infinity
        
        inv_mask = torch.unsqueeze(torch.unsqueeze(torch.logical_not(att_mask),-1),-1) 
        mask_B = inv_mask.permute((0, 2, 3, 1)) 
        score = score.to(x.device) - mask_B.to(x.device) * self.infinity
        ...
        o = torch.abs(o * station_mask.to(x.device))
        return o
...
class FullModel(nn.Module):
    def __init__(self, device, input_shape, downsample, mlp_dims, wavelength, total_station,
        ...
        self.SegmentEmbedding = nn.Embedding(250,embedding_dim=500)
        nn.init.zeros_(self.SegmentEmbedding.weight.data)
    def forward(self, waveforms, inputs,embedding_input):
        x, att_mask, station_mask = self.TotalEmbedding(waveforms, inputs,embedding_input)
...
def mixture_density_loss(y_true, y_pred, device='cpu',  eps=1e-6, d=1, mean=True, print_shapes=False):
    ...
    alpha = y_pred[:, :, 0]
    density = torch.ones(y_pred.shape[0], y_pred.shape[1]).to(device)
    for j in range(d):
        mu = y_pred[:, :, j + 1]
        sigma = y_pred[:, :, j + 1 + d]
...
def time_distributed_loss(y_true, y_pred, loss_func, device='cpu', norm=1, mean=True, summation=True, kwloss={}):
    ...
    loss = loss_func(y_true, y_pred, device, **kwloss)  
...
class EnsembleEvaluateModel:
    def __init__(self, config, experiment_path, max_ensemble_size=None, loss_limit=None, batch_size=64, device='cpu'):
        ...
        for ens_id in range(self.ensemble):
            ...
            tmp_model = torch.nn.DataParallel(tmp_model).to(device)
    ...
    def predict_generator(self, generator, **kwargs):
        ...
        for i, model in enumerate(self.models):
            dataloader = DataLoader(generator, batch_size=None , collate_fn=my_collate,num_workers=10)
            ...
            with torch.no_grad():
                ...
                for data_idx, data in enumerate(dataloader): 
                    ...
                    s = torch.unsqueeze(torch.arange(0,250), 0).to(self.device).to(non_blocking=True)
                    s = s.repeat(waveforms.shape[0], 1)
                    outputs = model(waveforms, inputs, s)  
